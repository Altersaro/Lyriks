  ...
  <div className="w-full flex flex-row items-center hover:bg-[#4c426e] py-2 p-4 rounded-lg cursor-pointer mb-2">
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      <img
        ...
        src={song.images?.coverart ? song.images?.coverart : '../../song-not-found.jpg'}
        alt={song?.title}
      />
      <div className="flex-1 flex flex-col justify-center mx-3">
        ...
        <Link to={`/artist/${song?.artists[0].adamid}`}>
          <p className="text-ase  text-gray-500">{song?.subtitle}</p>
        </Link>
...
const TopPlay = () => {
  ...
  useEffect(() => {
    ...
    setTopPlays(data || { tracks: [{ title: 'Loading...', artists: [{}], images: { background: [] } }] });
  }, [data]);

  const topFivePlays = topPlays.tracks?.slice(0, 5);
